...
def HaganImpliedVolatility(K,T,f,alpha,beta,rho,gamma):
 ...
 A = alpha/(np.power(f*K,((1.0-beta)/2.0))*(1.0+np.power(1.0-beta,
2.0)/24.0*
...
24.0*gamma*gamma)*T
 ...
 alpha)/np.power(f,(1.0-beta))+(2.0-3.0*rho*rho)/
24.0*gamma*gamma)*T
...
1
 ...
 d2sigmadx2 = lambda x,t: (sigma(x+dx,t) + sigma(x-dx,t)-2.0*sigma(x,t))/
(dx*dx)
 ...
 #d2omegadx2 = lambda x,t: 2.0*t*(dsigmadx(x,t))**2.0 +
2.0*t*sigma(x,t)*d2sigmadx2(x,t)
 d2omegadx2 = lambda x,t: 2.0*t*np.power(dsigmadx(x,t),2.0) +
2.0*t*sigma(x,t)*d2sigmadx2(x,t)
 term1 = lambda x,t: 1.0+x*domegadx(x,t)*(0.5 - np.log(x/
(s0*np.exp(r*t))) / omega(x,t))
 term2 = lambda x,t: 0.5*np.power(x,2.0)*d2omegadx2(x,t)
 term3 = lambda x,t: 0.5*np.power(x,2.0)*np.power(domegadx(x,t),
2.0)*(-1.0/8.0-1.0/(2.0*omega(x,t))\
 +np.log(x/(s0*np.exp(r*t)))*np.log(x/(s0*np.exp(r*t)))/
(2*omega(x,t)*omega(x,t)))
 # Final expression for local variance
 sigmalv2 = lambda x,t:(domegadt(x,t)+r*x*domegadx(x,t))/(term1(x,t)
+term2(x,t)+term3(x,t))
 return sigmalv2
...
2
 func = lambda sigma: np.power(BS_Call_Put_Option_Price(CP,S_0,K,sigma,T,r)
- marketPrice, 1.0)
...
def mainCalculation():
 ...
 sigma = lambda x,t:
HaganImpliedVolatility(x,t,f_0,alpha,beta,rho,volvol)
...
3
 ...
 # Stock path
 S[:,i+1]=S[:,i] * (1.0 + r*dt +
sigmaLV.transpose()*Z[:,i]*np.sqrt(dt))

 # We force that at each time S(t)/M(t) is a martingale
 S[:,i+1]= S[:,i+1] - np.mean(S[:,i+1]) + s0*np.exp(r*time[i])

 # Make sure that after moment matching we don't encounter negative
stock values
