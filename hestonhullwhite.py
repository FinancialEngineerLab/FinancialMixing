...
def CallPutOptionPriceCOSMthd_StochIR(cf,CP,S0,tau,K,N,L,P0T):
 # cf - Characteristic function is a function, in the book denoted by
\varphi
...
def Chi_Psi(a,b,c,d,k):
 psi = np.sin(k * np.pi * (d - a) / (b - a)) - np.sin(k * np.pi * (c - a)/
(b - a))
 psi[1:] = psi[1:] * (b - a) / (k[1:] * np.pi)
 psi[0] = d - c

 chi = 1.0 / (1.0 + np.power((k * np.pi / (b - a)) , 2.0))
 expr1 = np.cos(k * np.pi * (d - a)/(b - a)) * np.exp(d) - np.cos(k *
np.pi
 * (c - a) / (b - a)) * np.exp(c)
...
def ImpliedVolatilityBlack76(CP,marketPrice,K,T,S_0):
 ...
 func = lambda sigma: np.power(BS_Call_Put_Option_Price(CP,S_0,K,sigma,T,
0.0) - marketPrice, 1.0)
...
3
 f_2=1.0/c_1*(1.0-np.exp(-c_1*tau))+1.0/(c_1+lambd)*(np.exp(-
(c_1+lambd)*tau)-1.0)
 f_3=(np.exp(-(c_1+d)*tau)-1.0)/(c_1+d)+(1.0-np.exp(-(c_1+d+lambd)*tau))/
(c_1+d+lambd)
 f_4=1.0/c_1-1.0/(c_1+d)-1.0/(c_1+lambd)+1.0/(c_1+d+lambd)
 f_5=np.exp(-(c_1+d+lambd)*tau)*(np.exp(lambd*tau)*(1.0/(c_1+d)-
np.exp(d*tau)/c_1)+np.exp(d*tau)/(c_1+lambd)-1.0/(c_1+d+lambd))
...
def A(u,tau,eta,lambd,Rxsigma,Rrsigma,Rxr,gamma,kappa,sigmabar):
 ...
 d =np.sqrt(a_1*a_1-4.0*a_0*a_2)
 g =(-a_1-d)/(-a_1+d)
 f_6=eta*eta/(4.0*np.power(lambd,3.0))*np.power(i+u,
2.0)*(3.0+np.exp(-2.0*lambd*tau)-4.0*np.exp(-lambd*tau)-2.0*lambd*tau)
 ...
 f=(kappa*sigmabar+1.0/2.0*gamma*gamma*E_val
+gamma*eta*Rrsigma*C_val)*E_val
 value1 =integrate.trapz(np.real(f),arg)
 value2 =integrate.trapz(np.imag(f),arg)
 value[k]=(value1 + value2*i)

 return value + A_1
def ChFSZHW(u,P0T,sigma0,tau,lambd,gamma,
Rxsigma,Rrsigma,Rxr,eta,kappa,sigmabar):
 ...
 hlp = eta*eta/(2.0*lambd*lambd)*(tau+2.0/lambd*(np.exp(-
lambd*tau)-1.0)-1.0/(2.0*lambd)*(np.exp(-2.0*lambd*tau)-1.0))
 ...
 *(kappa-gamma*rhoxv*i*u-D1)
 return C
def
ChFH1HW_FX(u,tau,gamma,Rxv,Rxrd,Rxrf,Rrdrf,Rvrd,Rvrf,lambdd,etad,lambdf,etaf,k
appa,vBar,v0):
 i = np.complex(0.0,1.0)
...
0.5*(etad**2.0*Bd(t,tau)**2.0+etaf**2.0*Bf(t,tau)**2.0)
...
def
calibrationH1HW_FX_Global(CP,marketPrice,P0Td,T,K,frwdFX,Rxrd,Rxrf,Rrdrf,Rvrd,
Rvrf,lambdd,etad,lambdf,etaf,kappa):
 K = np.array(K)
 marketPrice = np.array(marketPrice)
 # x = [gamma,vBar,Rxv,v0]
 f_obj = lambda x:
TargetValH1HW_FX(CP,marketPrice,P0Td,T,K,frwdFX,x[0],x[1],x[2],x[3],Rxrd,Rxrf,
Rrdrf,Rvrd,Rvrf,lambdd,etad,lambdf,etaf,kappa)
 ...
 minimizer_kwargs = dict(method="L-BFGS-B", bounds=bounds,options={'niter':
1})
 # Global search
 pars = optimize.basinhopping(f_obj, initial,niter=1,
minimizer_kwargs=minimizer_kwargs)
 ...
 return parmCalibr
def
TargetValH1HW_FX(CP,marketPrice,P0Td,T,K,frwdFX,gamma,vbar,Rxv,v0,Rxrd,Rxrf,Rr
drf,Rvrd,Rvrf,lambdd,etad,lambdf,etaf,kappa):
 if K is not np.array:
6
 K = np.array(K).reshape([len(K),1])
 ...
 cf = lambda u:
ChFH1HW_FX(u,T,gamma,Rxv,Rxrd,Rxrf,Rrdrf,Rvrd,Rvrf,lambdd,etad,lambdf,etaf,kap
pa,vbar,v0)

 valCOS = P0Td(T)*CallPutOptionPriceCOSMthd_StochIR(cf, CP, frwdFX, T, K,
N, L,1.0)
...
7
 ...
 referencePrice = P0Td(T)*
BS_Call_Put_Option_Price(CP,frwdFX,K,referenceIV,T,0.0)
 ...
 calibratedParms =
calibrationH1HW_FX_Global(CP,referencePrice,P0Td,T,K,frwdFX,Rxrd,Rxrf,Rrdrf,Rv
rd,Rvrf,lambdd,etad,lambdf,etaf,kappa)
...
ChFH1HW_FX(u,T,gamma,Rxv,Rxrd,Rxrf,Rrdrf,Rvrd,Rvrf,lambdd,etad,lambdf,etaf,kap
pa,vBar,v0)
 valCOS_H1HW = P0Td(T)*CallPutOptionPriceCOSMthd_StochIR(cf, CP, frwdFX, T,
K, N, L,1.0)

 plt.plot(K,valCOS_H1HW,'--r')
 plt.legend(['Input','Calibration output'])

 print("Optimal parameters for HHW-FX are: gamma = {0:.3f}, vBar = {1:.3f},
...
====")
 ...
 IVMarket[idx] =
ImpliedVolatilityBlack76(CP,priceMarket,k,T,frwdFX)*100.0
